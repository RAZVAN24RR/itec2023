...
const ProfilePage = () => {
  ...
  const calculateTechnicalSkillsScore = (existingSkills, newSkills) => {
    const matchedSkills = existingSkills.filter(skill => newSkills.includes(skill));
  ...
  const calculateSoftSkillsScore = (existingSkills, newSkills) => {
    ...
    return matchedSkills.length / newSkills.length; // normalize to a score between 0 and 1
        ...
        for (let i = 0; i < existingEmployees.length; i++) {
          ...
          const matchScore = (
            calculateWorkExperienceScore(existingEmployee.workExperience, newEmployeeSkills.workExperience) +
            calculateTechnicalSkillsScore(existingEmployee.technicalSkills, newEmployeeSkills.technicalSkills) +
            calculateSoftSkillsScore(existingEmployee.softSkills, newEmployeeSkills.softSkills)
          );
          ...
          <ul>
            ...
            <li>HTML/CSS</li>
             ...
             {applicationStatus.status === "Pending" ? (
               <p>Your application is currently being processed. Please wait for a response.</p>
